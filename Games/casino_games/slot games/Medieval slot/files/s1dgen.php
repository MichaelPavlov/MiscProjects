...
function Gen() { 
	...
	if ( ($res >= 175) && ($res < 250) ) { $n = 6; }
	if ( ($res >= 250) && ($res < 325) ) { $n = 5; }
	if ( ($res >= 325) && ($res < 400) ) { $n = 4; }
...
function countLine($a, $b, $c){
	...
	if( ($a==$b)&&($b==$c) ){
		switch ($a){
			...
			case 4: $win = 75 * $bet; break;
			case 5: $win = 50 * $bet; break;
			case 6: $win = 25 * $bet; break;
			case 7: $win = 5 * $bet; break;
			//case 8: $win = bonus; break; // Bonus game here
		}
	}else{
		if ( ($a == $b) && ($b == 4) || ($b == $c) && ($b == 4) || ($a == $c) && ($a == 4)) { 
			$win = 2 * $bet; 
		} //2x
		elseif (($a == 4) || ($b == 4) || ($c == 4)) {
			$win = $bet;
